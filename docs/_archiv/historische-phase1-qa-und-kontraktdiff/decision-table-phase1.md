...
| --- | --- | --- | --- | --- |
| Angebots-Aenderungen (Lebenszyklus) | A) Nach `VERSENDET` sofort fixiert; Korrektur nur neue Version/Nachtrag<br>B) `ENTWURF` und `VERSENDET` (ohne Annahme) anpassbar; ab `ANGENOMMEN` ausschliesslich Nachtrag | **B** | A | Entspricht `../systembeschreibung-und-phasen-legacy/ERP Systembeschreibung v1.2.md`. Trade-off: Weicht von einer strikten „Versand fixiert alles“-Lesart ab; dafuer fachlich gewollte Nachbearbeitung bis zur Annahme. Rechtliche Bindung nach Zugang beim Kunden bleibt mandanten-/vertragsspezifisch (vgl. Annahme Abschnitt 16 in v1.2). |
