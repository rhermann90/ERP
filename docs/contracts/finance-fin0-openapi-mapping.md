...
- **`POST /finance/payments/intake`:** Header wird **case-insensitive** gelesen; Wert muss **UUID** sein — sonst `400` / `VALIDATION_FAILED` (Zod), konsistent zu `docs/api-contract.yaml` (`components.parameters.IdempotencyKey`; `responses` für denselben Pfad) und `docs/contracts/qa-fin-0-stub-test-matrix.md`.
