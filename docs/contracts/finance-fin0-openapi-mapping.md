...
| **FIN-2** | `POST /invoices` (optional `measurementId` bei mehreren Aufmassen), `GET /invoices/{invoiceId}`, `POST /invoices/{invoiceId}/book` | `src/api/finance-invoice-routes.ts`, `src/services/invoice-service.ts`, `src/persistence/invoice-persistence.ts` |
...
| **FIN-5 (Shell-SoT Schreibpfad)** | *(SoT)* `GET /documents/{invoiceId}/allowed-actions?entityType=INVOICE` liefert **`MANAGE_INVOICE_TAX_SETTINGS`** bei allen Rechnungsstatus und Rollen wie `assertCanManageInvoiceTaxSettings` (gleiche Menge wie Zahlungseingang) | `src/services/authorization-service.ts`; PWA Finanz-Vorbereitung: `apps/web/src/components/finance/preparation/FinanceInvoiceTaxSettingsPanel.tsx` (**PATCH** `/finance/invoice-tax-profile`, **PUT|DELETE** Projekt-Override); **`docs/contracts/action-contracts.json`** |
