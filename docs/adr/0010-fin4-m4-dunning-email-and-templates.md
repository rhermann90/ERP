...
**Status dieses Abschnitts:** **Accepted** für **Phase 5b-0** (2026-04-23) und **Phase 5b-1** (2026-04-24). PL-Entscheide: Ticket [`docs/tickets/M4-MINI-SLICE-5B-ORCHESTRATION-2026-04-24.md`](../tickets/M4-MINI-SLICE-5B-ORCHESTRATION-2026-04-24.md). **5b-2** (Hintergrund-Cron) ist **Accepted** als technischer **optionaler** Betriebspfad (2026-04-24): `POST /internal/cron/dunning-automation` nur mit gesetztem `ERP_INTERNAL_DUNNING_CRON_SECRET` + Postgres; **kein** Batch-E-Mail; Massen-E-Mail weiter Slice **5a** / PL.
...
|-------|------------------|------------------|
| **5b-0** (Accepted) | `GET /finance/dunning-reminder-candidates?stageOrdinal={1–9}&asOfDate={yyyy-mm-dd optional}` | `assertCanReadInvoice` (wie Vorlagen/Footer). **Keine** Mutation. Antwort-Envelope `{ data: { configSource, asOfDate, stageOrdinal, daysAfterDueForStage, candidates[] } }`. Jedes Listenelement: `invoiceId`, `dueDate` (siehe unten), `openAmountCents`, optional `lastDunningStageOrdinal` (fehlt = noch kein Mahn-Ereignis). |
| **5b-1** (Accepted) | `POST /finance/dunning-reminder-run` mit JSON-Body `{ stageOrdinal, reason, mode: "DRY_RUN" \| "EXECUTE", asOfDate?: yyyy-mm-dd, invoiceIds?: uuid[], note?: string }` | **`DRY_RUN`:** `assertCanReadInvoice`; **keine** Mutation; Antwort listet geplante Mahnzeilen je `invoiceId` (gleiche Eligibility wie 5b-0). **`EXECUTE`:** `assertCanRecordDunningReminder`; Header **`Idempotency-Key`** (UUID, Pflicht); schreibt pro Rechnung ein Mahn-Ereignis wie `POST /invoices/{invoiceId}/dunning-reminders` (**kein** Batch-Aufruf von Slice **5a** `send-email` in dieser Lieferung). Optionaler Body `asOfDate` wie 5b-0 (Default heute UTC). `invoiceIds` optional: Teilmenge der Kandidaten; wenn weggelassen → alle Kandidaten dieser Stufe. Sortierung deterministisch nach `invoiceId`. |
| **5b-2** (Accepted, Betrieb optional) | Interner **Cron**-Einstieg `POST /internal/cron/dunning-automation` | Header **`X-Internal-Cron-Secret`** muss zu `ERP_INTERNAL_DUNNING_CRON_SECRET` passen (401 sonst). Verarbeitet Mandanten mit persistiertem **`run_mode = AUTO`** (`dunning_tenant_automation`): optional `job_hour_utc` (0–23 UTC) — wenn gesetzt, läuft der Tick nur in dieser Stunde; wenn **null**, bei jedem Aufruf. Pro Mandant und Stufe 1–9: bei Kandidaten `EXECUTE` wie 5b-1 mit **stabiler** Idempotenz pro (`tenantId`, `stageOrdinal`, `asOfDate`). **Kein** SMTP / kein Batch-5a. |

### Mandanten-Automation (Modus, persistiert)

1. **Tabelle** `dunning_tenant_automation` (`tenant_id` PK, `run_mode` **OFF** \| **SEMI** \| **AUTO**, optional `job_hour_utc` 0–23 UTC, `updated_at`).
2. **HTTP:** `GET /finance/dunning-reminder-automation` — `assertCanReadInvoice`; Antwort `{ data: { automationSource: NOT_CONFIGURED \| TENANT_DATABASE, tenantId, runMode, jobHourUtc } }` — ohne Zeile: `NOT_CONFIGURED` und effektiv **SEMI** (UI darf Batch nutzen; kein Cron).
3. **HTTP:** `PATCH /finance/dunning-reminder-automation` — `assertCanManageDunningTenantStageConfig`; Body `{ reason, runMode, jobHourUtc? }`; nur Postgres (`503` `DUNNING_AUTOMATION_NOT_PERSISTABLE` im In-Memory-Modus); Audit **`DUNNING_TENANT_AUTOMATION_PATCHED`** (`entityType` `DUNNING_TENANT_STAGE_CONFIG`, `entityId` = `tenantId`).
...
- **Fälligkeitsanker (MVP):** Es existiert im Domain-Modell noch **kein** persistiertes `dueDate`. **Referenzdatum für „fällig“** = `issueDate` (**ISO `yyyy-mm-dd`**, Rechnungsdatum). Mahnfristende = `issueDate` + **`daysAfterDue`** der **Zielstufe** (Kalendertage **UTC**, keine Feiertagslogik). Bedingung: Fristende **≤** `asOfDate` (Query; Default: **heute UTC** als `yyyy-mm-dd`).
...
- **Slice 5b:** **5b-0**, **5b-1** und der **optionale** Cron-**5b-2** sind **Accepted** und ticketgebunden — [`docs/tickets/M4-MINI-SLICE-5B-ORCHESTRATION-2026-04-24.md`](../tickets/M4-MINI-SLICE-5B-ORCHESTRATION-2026-04-24.md); Betrieb: [`docs/runbooks/dunning-automation-cron.md`](../runbooks/dunning-automation-cron.md).
