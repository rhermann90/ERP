...
| **FIN-6 §15 Vorlage** | [`docs/contracts/qa-fin6-section15-acceptance.md`](../contracts/qa-fin6-section15-acceptance.md) | Technische Abnahme-Checkliste (Team-Checkboxen) |
...
| Datei / Muster | Inhalt |
|----------------|--------|
| `*-routes.ts` | Ressourcen-Routen (LV, Aufmass, Angebot, Finanz, Auth, Nutzer, …); **`finance-invoice-tax-routes`** (`/finance/invoice-tax-profile…`); **`finance-e-invoice-party-routes`** (`/finance/e-invoice-parties…`, XRechnung Seller/Buyer); **`tenant-pwa-display-settings-routes`** (`GET|PATCH /tenant/pwa-display-settings`, Mandanten-Expertenmodus PWA); **`GET /users`** paginiert (`user-account-routes` / `user-account-service`); **`GET /lv/versions/{lvVersionId}`** Lesepfad §9 → `lv-service.getVersionSnapshot`; **`GET .../structure`** + **`GET .../nodes/{nodeId}`** + **`GET .../positions/{positionId}`** → `lv-hierarchy-service` (Projektion; ADR-0013) |
