...
4. **Plan „Nächste Entwicklungsschritte“ (Repo / Agent):** Mit Punkt 2 abgedeckt (`verify:pre-merge`); bei Bedarf weiterhin einzeln `npm run verify:ci` und `npx playwright test e2e/login-finance-smoke.spec.ts`. **Slice 5c / Mandanten-Go Massen-E-Mail** — **Mandanten-Produktiv-Go**, nicht Merge: Agenda [`m4-slice-5c-pl-mandanten-go.md`](../runbooks/m4-slice-5c-pl-mandanten-go.md) *(Pfadname historisch)* und Checkliste [`compliance-rechnung-finanz.md`](../../Checklisten/compliance-rechnung-finanz.md); **kein** Repo-Ersatz für StB/DSB/Release-Verantwortliche (siehe **Team-Touchpoint** unten; [AGENTS.md](../../AGENTS.md) Punkt 6). **Optional:** weitere E2E oder Shell-**GET** nur mit klarer OpenAPI in [`api-contract.yaml`](../api-contract.yaml), **eigener PR**, strikt getrennt von Finanz-Schreibpfaden — **Parallele Spur** unten und [`workflow-code-first-ohne-qualitaetsverlust.md`](./workflow-code-first-ohne-qualitaetsverlust.md).
...
**Gewählte Spur (Team):** **B — FIN-5 §8.16 Option A (Folge-Inkremente)** seit Merge [#86](https://github.com/rhermann90/ERP/pull/86) (2026-05-04). Backlog der nächsten Pakete: [`FIN-5-FOLLOWUP-INCREMENTS.md`](../tickets/FIN-5-FOLLOWUP-INCREMENTS.md). **Primärer** nächster Implementierungsstrang ist damit **B** (ein Paket pro PR aus diesem Backlog). Spur **A** (Option A / M4-Rest: Copy, optionale Shell-`GET`, Skonto-PWA, Mahn-UX) bleibt **parallel in kleinen, isolierten PRs** möglich — **nicht** zusammen mit FIN-5-/B5-/FIN-6-Fläche im selben PR (Non-Goals in [`NEXT-INCREMENT-FINANCE-WAVE3.md`](../tickets/NEXT-INCREMENT-FINANCE-WAVE3.md) und im Folge-Ticket).

**Historischer Kontext (Plan „Nächstes großes Inkrement“, 2026-05-01):** Damals **Zweig 2** unter Spur **A** (Doku-Konsistenz NEXT-INCREMENT vs. M4 Slice 5c) und **Zweig 1** als strategischer FIN-5-Anker „nach Teamentscheid §8.16 vs. Fail-Closed“. — **Superseded durch IST:** Merge [#86](https://github.com/rhermann90/ERP/pull/86) hat **Option A** mit [ADR-0015](../adr/0015-fin5-invoice-tax-regimes-816.md) umgesetzt; **Gewählte Spur** ist oben auf **B** gesetzt; Nachzug-Doku [#87](https://github.com/rhermann90/ERP/pull/87).

**Aktueller Auftrag (Repo):** Nächste Implementierungs-PRs für FIN-5 aus [`FIN-5-FOLLOWUP-INCREMENTS.md`](../tickets/FIN-5-FOLLOWUP-INCREMENTS.md) (Pakete A–D). Risiken und Mitigation dort im Abschnitt **Risiken und Mitigation**.
...
- **Agent (2026-04-28):** [`compliance-rechnung-finanz.md`](../../Checklisten/compliance-rechnung-finanz.md) enthält den Punkt **FIN-4 / Massen-E-Mail (M4 Slice 5c)**; Agenda-Vorschlag unten bleibt kanonisch — **Mandanten-Produktiv-Go** für 5c weiterhin **Release-Verantwortliche + StB/DSB** (kein Repo-Ersatz).
- **Ziel:** Tempo durch **Entscheidung**, nicht durch parallele Groß-PRs.
- **Agenda-Vorschlag (copy-paste):** (1) **M4 Slice 5c** (Massen-E-Mail) ist im Repo umgesetzt — **Mandanten-Produktiv** ja/nein und SMTP/Idempotenz-Betrieb klären; (2) vor Live-Schaltung **5c** die Checkliste [`compliance-rechnung-finanz.md`](../../Checklisten/compliance-rechnung-finanz.md) mit **StB / DSB / Release-Verantwortliche**; (3) weiteres Mahn-UX nach Ticket-Priorität — [`FOLLOWUP-M4-DUNNING-UX-GRUNDEINSTELLUNGEN-TAB.md`](../tickets/FOLLOWUP-M4-DUNNING-UX-GRUNDEINSTELLUNGEN-TAB.md).
