...
- **Repo (Agent 2026-05-01 UTC):** Demo-Seed um `SupplementOffer` / `SupplementVersion` ergänzt ([`src/composition/seed.ts`](../../src/composition/seed.ts)); Haupt-Shell `data-testid="supplement-shell-detail"`; Playwright **SUPPLEMENT_VERSION** in [`e2e/login-finance-smoke.spec.ts`](../../e2e/login-finance-smoke.spec.ts); [`apps/web/src/lib/demo-seed-ids.ts`](../../apps/web/src/lib/demo-seed-ids.ts) — `supplementVersionId`. Keine neue P1-3-Meilenstein-Zeile ohne qualifizierenden Finanz-Merge; PR mit **`backend`** + **`e2e-smoke`** wie üblich.
