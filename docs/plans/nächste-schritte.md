...
**Erledigt / präzisiert (Schritt 4 — Shell):** `GET /invoices/{id}` read-only für **INVOICE** in der Haupt-Shell über `ApiClient.getInvoice`; **`GET /lv/versions/{lvVersionId}`** für **LV_VERSION** („Detail (GET)“) — siehe [`apps/web/src/App.tsx`](../../apps/web/src/App.tsx). Optional bleiben: weitere Shell-`GET`-Typen, sobald öffentliche Routen und OpenAPI feststehen (getrennt von Finanz-Schreibpfaden).
...
- **Skonto-UI:** optional nach API-First / Team-Priorität — Non-Goal-Hinweis in [`docs/tickets/NEXT-INCREMENT-FINANCE-WAVE3.md`](../tickets/NEXT-INCREMENT-FINANCE-WAVE3.md).
- **Haupt-Shell:** Rechnung read-only (`GET /invoices/…`); LV-Version read-only (`GET /lv/versions/…`, Entity **LV_VERSION**); Rauchtest [`e2e/login-finance-smoke.spec.ts`](../../e2e/login-finance-smoke.spec.ts). Optional: weitere Lesetypen nach OpenAPI.
