...
- **Agent (Phase 2 LV §9 Inc2, 2026-05-04 UTC):** Branch `feat/phase2-lv-section9-domain-and-contracts` — read-only **`GET /lv/versions/{id}/structure`**, **`GET /lv/versions/{id}/positions/{pid}`**, Domäne/Services/Seed/Memory, ADR-0013, OpenAPI **`1.28.1-phase2-lv-section9-hierarchy`**; `npm run verify:ci`, `verify:ci:local-db`, Playwright `e2e/login-finance-smoke.spec.ts` wie üblich vor Merge. **P1-3:** Nach **Merge auf `main`** (PR [#82](https://github.com/rhermann90/ERP/pull/82)) verbindlich **eine** neue Meilenstein-Zeile in [`P1-3-DOCS-MILESTONE-WAVE3.md`](../tickets/P1-3-DOCS-MILESTONE-WAVE3.md) — Kriterium **`docs/api-contract.yaml`** / **`info.version`** laut Abschnitt **Pflege (Agent)**; FIN4-Begleitung im PR ist mitgeführt; **keine** Tabellenzeile vor Merge ohne echte PR-/Merge-URL.
