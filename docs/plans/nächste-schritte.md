...
- **Agent (PWA Finanz-Vorbereitung Polish, 2026-05-04 UTC):** Branch `feat/web-finance-prep-a11y-error-polish` — Finanz-Vorbereitung: strukturierte API-Fehler durchgängig (`FinanceStructuredApiError`, `extractStructuredError`/`finNoticeFromUnknown`), zentrale Copy `FIN_PREP_ERROR_COPY`, A11y Step-Sections/`aria-live`, E2E Tenant-Mismatch + Tab-Fokus in [`e2e/login-finance-smoke.spec.ts`](../../e2e/login-finance-smoke.spec.ts); **kein** Backend/OpenAPI-Diff in diesem PR. **P1-3:** Nach **Merge auf `main`** (PR [#83](https://github.com/rhermann90/ERP/pull/83)) verbindlich **eine** neue Meilenstein-Zeile in [`P1-3-DOCS-MILESTONE-WAVE3.md`](../tickets/P1-3-DOCS-MILESTONE-WAVE3.md) — Kriterium **`apps/web`** Finanz-Vorbereitung laut **Pflege (Agent)**; vor Merge `npm run verify:pre-merge`; **keine** Tabellenzeile vor Merge ohne echte PR-/Merge-URL.
