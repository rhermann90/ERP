...
- **Performance:** Bei neuen Listen-Endpunkten auf LIMIT/Pagination achten (Skill-Hinweis N+1/unbounded) — **`GET /users`** ist seit Nacharbeit paginiert (OpenAPI **1.28.5-users-list-paged**); andere kritische Pfade nicht im Vollumfang vermessen in diesem Audit.
...
| Check | Ergebnis |
|--------|----------|
| `npm run verify:pre-merge` (= `verify:ci` + Playwright Finanz-Smoke [`e2e/login-finance-smoke.spec.ts`](../../e2e/login-finance-smoke.spec.ts) + App-Shell-Smoke [`e2e/app-shell-smoke.spec.ts`](../../e2e/app-shell-smoke.spec.ts)) | **Nachziehen:** zweite Spec ergänzt leichte „Full exploration light“-Abdeckung (Login-/Reset-Shell ohne Secrets); Gesamtzahl E2E-Tests nach lokalem Lauf von `verify:pre-merge` verwenden |
| Explorationsplan (Skill „Full exploration“) | **Teilweise:** App-Shell-Rauchtest ohne Login; vollständige UI-Exploration weiterhin optional bei großen PWA-Änderungen |

Merge-Evidenz-Vorlagen: [`docs/contracts/qa-fin-0-gate-readiness.md`](../contracts/qa-fin-0-gate-readiness.md).

### Umgesetzt / verbleibend (Nacharbeit Risiko-Plan)

- **Umgesetzt:** `GET /users` paginiert (`data`, `page`, `pageSize`, `total`; Query `page`, `pageSize`); OpenAPI/`info.version` **1.28.5-users-list-paged**; Persistenztests; `verify:pre-merge` inkl. App-Shell-E2E; AGENTS / PR-Vorlage / Runbook / `authentication-login.md` / FIN4-Integratorenhinweis zu **1.28.5**.
- **Verbleibend (strategisch):** FIN-5 §8.16 vs. Fail-Closed — Team-Entscheidung, keine Produktcode-Festlegung durch dieses Follow-up ([`FIN-5-GATE-816-FAIL-CLOSED.md`](../tickets/FIN-5-GATE-816-FAIL-CLOSED.md), ADR [`0014-fin5-mvp-tax-fail-closed.md`](../adr/0014-fin5-mvp-tax-fail-closed.md)).
- **`$executeRawUnsafe` / Readiness:** unverändert — weiterhin nur Hinweis unter Rohe SQL-Stellen (Abschnitt 3).
...
- **QA optional:** Vollständige UI-Exploration (Skill „Full exploration“) bei großen PWA-Änderungen zusätzlich zu den Smoke-Specs — App-Shell-Smoke schließt die größte Lücke für unauthentifizierte Routen nicht vollständig.
