...
| **G7** | **CI / Tests:** Auf dem Referenz-Branch (z. B. `main`) sind **`npm test`** (Root-Suite, Persistenz) und **`npm run test -w apps/web`** (PWA-Workspace) bzw. die projektübliche CI-Kette **grün**, inkl. Persistenz-Jobs, sofern für LV/Aufmass-Slice konfiguriert. | ja | Workflow: [`.github/workflows/ci.yml`](../../.github/workflows/ci.yml) — Job `backend`: `PERSISTENCE_DB_TEST_URL`, `prisma migrate deploy`, `npm run build:web`, **`npm run test -w apps/web`**, `npm test` (Persistenz-Suite ohne SKIP). Lokal spiegelt [`package.json`](../../package.json) `verify:ci` dieselbe Kette (ohne `migrate deploy`; dafür `verify:ci:with-migrate`). Aktuelle Runs: GitHub Actions **CI** auf `main` / PRs gegen `main`. |
