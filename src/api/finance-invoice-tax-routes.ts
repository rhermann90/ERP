...
const invoiceTaxReadRateLimit = {
  max: 60,
  timeWindow: "1 minute" as const,
};

const invoiceTaxWriteRateLimit = {
  max: 20,
  timeWindow: "1 minute" as const,
};
...
export function registerFinanceInvoiceTaxRoutes(
  ...
  app.get(
    ...
    { config: { rateLimit: invoiceTaxReadRateLimit } },
    async (request, reply) => {
      try {
  ...
  app.patch(
    "/finance/invoice-tax-profile",
    { config: { rateLimit: invoiceTaxWriteRateLimit } },
    async (request, reply) => {
      try {
        const auth = parseAuthContext(request.headers);
  ...
  app.get(
    "/finance/invoice-tax-profile/projects/:projectId",
    { config: { rateLimit: invoiceTaxReadRateLimit } },
    async (request, reply) => {
      try {
        const auth = parseAuthContext(request.headers);
