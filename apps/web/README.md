...
**Troubleshooting CRM-Stamm (`/crm/…`, z. B. Stammdaten-Hub):** Meldungen wie **„Route GET:/crm/customers not found“** (HTTP 404 vom Server) bedeuten fast immer, dass die **antwortende Backend-Instanz** noch einen **älteren Build** ohne CRM-Routen fährt oder die PWA **falsches API-Origin** nutzt (nicht `:5173` ohne Proxy — siehe `vite.config.ts` ohne `server.proxy`). Prüfen: gleiche Basis-URL wie in der Shell, `GET {apiBase}/health`, Response-Header **`x-erp-openapi-contract-version`** gegen `docs/api-contract.yaml` `info.version` / Backend `src/domain/openapi-contract-version.ts`. Backend **neu starten** bzw. Deploy aktualisieren; bei Postgres Migration **`20260507120000_crm_stammdaten_option_c`** anwenden (`prisma migrate deploy`). **503** mit Code **`CRM_PERSISTENCE_UNAVAILABLE`** ist erwartbar im **Memory-Modus** ohne Postgres — das ist kein „Route fehlt“, sondern CRM nur mit DB.
