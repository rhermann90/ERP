...
**Reihenfolge:** 1) PL-Vorspann in jede Session → 2) Backend → 3) Frontend → 4) QA → 5) Senior Code Review.  
**So kopieren:** Pro Rolle nur den **Inhalt** des jeweiligen Kastens kopieren — **ohne** die erste und letzte Zeile des Kastens (die Zeilen, die jeweils nur drei Backticks und ggf. `text` enthalten).  
**Rohdatei (gesamte Datei):** https://raw.githubusercontent.com/rhermann90/ERP/main/prompts/AGENTEN-RUNDE-2026-04-19-FIN0-nach-reviewer-rueckmeldung.md  

**Nächste Runde nach Code-Review:** Du fügst die **Rückmeldung an Projektleitung** (blocking wortgleich GitHub, Pflichtzeilen, Evidence) in den Chat ein — daraus wird die **nächste** `AGENTEN-RUNDE-…`-Datei und die **nächsten** fünf Blöcke erzeugt (`PL-NÄCHSTE-RUNDE-AUS-REVIEW.md` §2, `AGENTEN-PROMPT-LEITFADEN.md` §0).
...
## Doku — Ergebnis, Schritt 0, Vorlagen (nach den Chat-Blöcken lesen)
