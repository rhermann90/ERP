...
- **Postgres (Write-Through, `repositoryMode=postgres`):** Tabellen **`lv_*`**, **`measurement_*`**, **`offers`**, **`offer_versions`** (inkl. FK `offer_versions` → `lv_versions`, FIN-2-Gate **G5**), **`supplement_offers`** / **`supplement_versions`** (Nachtrag, FK zu Offers/OfferVersion/LV), **`users`** / **`password_reset_challenges`** (Login/Reset, siehe `docs/authentication-login.md`), sowie **`audit_events`** bei Audit-Schreibpfad. Startup-Reihenfolge: **LV + Aufmass synchronisieren, danach Offers, danach Supplements** (`src/api/app.ts`).
- **Ausschließlich In-Memory (keine Prisma-Tabellen in diesem Stand):** **`invoices`**, **`traceabilityLinks`**, **`exportRuns`**. Restart ohne Seed ⇒ diese Daten sind **nicht** aus Postgres wiederherstellbar.
- **Nächster Persistenz-Slice:** nur nach **Projektleitung-Priorität** und Ticket — z. B. vorbereitend **FIN-1** (Zahlungsbedingungen 8.3/8.5) **ausschließlich nach schriftlichem Go**; nicht parallel ohne Auftrag. **Nachtrag (`supplement_*`)** ist in Postgres umgesetzt (ADR-0002 D5); **FIN-2** (Rechnung / **8.4**-Motor): bis [`docs/tickets/FIN-2-START-GATE.md`](./docs/tickets/FIN-2-START-GATE.md) **G1–G10 = ja** **out of scope** (Gate-Pflege durch PL).
...
- **Audit-Events:** bei `repositoryMode=postgres` zusätzlich in **`audit_events`** (Migration); `GET /audit-events` bleibt DSGVO-minimiert. Details: [`docs/tickets/FOLLOWUP-AUDIT-PERSISTENCE.md`](./docs/tickets/FOLLOWUP-AUDIT-PERSISTENCE.md). **Review:** DB-Schreibfehler werden aktuell nur geloggt — Semantik vor Produktionslast: [`docs/tickets/FOLLOWUP-AUDIT-DB-PERSIST-FAIL-HARD.md`](./docs/tickets/FOLLOWUP-AUDIT-DB-PERSIST-FAIL-HARD.md).
